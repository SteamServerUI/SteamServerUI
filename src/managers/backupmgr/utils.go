...
func InitBackupMgr() {
	...
	cfg = Bckupcfg{
		BackupContentDir:   runfile.CurrentRunfile.GetBackupContentDir(),
	...
	logger.Backup.Debug("Content Directory: " + runfile.CurrentRunfile.GetBackupContentDir())
...
func IsSystemReady() bool {
	mu.Lock()
	defer mu.Unlock()
	return cfg.StoredBackupsDir != "" && cfg.BackupContentDir != ""
}
